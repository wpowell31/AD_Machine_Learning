...
# Load datasetActive/Will/ML_LLM_AD/AD_prediction/6months/df_extract_ad_prediction_no_meds.csv
df = pd.read_csv('/storage1/fs1/aditigupta/Active/Will/ML_LLM_AD/AD_prediction/1year/df_extract_ad_prediction_no_meds1yr.csv')
...
X = df.drop(columns=['EPIC_MRN', 'FIRST_ENC', 'LAST_ENC', 'DIFF_YEARS', 'INDEX_TIME_ENC',
       ...
       'PT_ZIPCODE', 'DIFF_YEARS', 'Memory loss', 'Dementias', 'Neurological disorders', 'Altered mental status',
       'Mild cognitive impairment', 'Central Nervous System (CNS) conditions', 'Dementia with cerebral degenerations', 'Vascular dementia',
       'screening for other diseases and disorders', 'Osteoporosis NOS'])
y = df['AD_label']
...
X = X.rename(
    columns = {
        "MDC_years": "Years seen in MDC",
        "age_at_index": "Age",
        "PT_GENDER": "Sex",
        "NUMBER_OF_MEDS_BEFORE_INDEX": "Number of Previous Medications",
        "POSITIVE_PET": "Positive Amyloid PET Scan",
        "POSITIVE_PTAU_ABETA": "Positive pTau217/Abeta42 test",
        "Hyperlipidemia_y": "Hyperlipidemia",
         "PT_RACE_White": "White",
        "repeat": "Repeats Statements (Notes Extraction)",
        "Cerebrovascular disease_x": "Cerebrovascular disease"
    }
)
...
for model_name in models:
    ...
    with open(f'/storage1/fs1/aditigupta/Active/Will/ML_LLM_AD/AD_prediction/1year/{model_name.lower()}_model_metrics_bootstrap_no_meds.txt', 'a') as f:
        ...
        f.write(f"Number of samples: {X.shape}")
        print(f"Hyperparameter Tuning Boostrap Resampling metrics saved to '{model_name.lower()}_model_metrics_bootstrap_no_meds.txt'")
    ...
    print("getting summary_plot")
    shap.summary_plot(shap_values, X_train, show=False)
    plt.savefig(f'/storage1/fs1/aditigupta/Active/Will/ML_LLM_AD/AD_prediction/1year/{model_name.lower()}_shap_summary_plot_bootstrap_no_meds.png', bbox_inches='tight')
    plt.close()
    print(f"SHAP summary plot saved to '{model_name.lower()}_shap_summary_plot_bootstrap_no_meds.png'")
