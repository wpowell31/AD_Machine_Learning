...
X = df.drop(columns=['EPIC_MRN', 'FIRST_ENC', 'LAST_ENC', 'DIFF_YEARS', 'INDEX_TIME_ENC',
       ...
       'PT_ZIPCODE', 'Memory loss', 'Dementias', 'Central Nervous System (CNS) conditions'])
y = df['AD_label']
...
for model_name in models:
    ...
    with open(f'/storage1/fs1/aditigupta/Active/Will/ML_LLM_AD/AD_prediction/{model_name.lower()}_model_metrics_bootstrap_female_sens.txt', 'a') as f:
        ...
        print(f"Hyperparameter Tuning Boostrap Resampling metrics saved to '{model_name.lower()}_model_metrics_bootstrap_female_sens.txt'")
    ...
    plt.savefig(f'/storage1/fs1/aditigupta/Active/Will/ML_LLM_AD/AD_prediction/{model_name.lower()}_shap_summary_plot_bootstrap_female_sens.png', bbox_inches='tight')
    plt.close()
    print(f"SHAP summary plot saved to '{model_name.lower()}_shap_summary_plot_bootstrap_female_sens.png'")
